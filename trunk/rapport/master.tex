\documentclass[final]{mypaper}

\newcommand{\bw}{true}

\ifthenelse{\boolean{\bw}}{
  \hypersetup{
    linkcolor=black,
    citecolor=black,
    filecolor=black,
    urlcolor=black
  }
}{}

% Nye komandoer og pakker - kk du kan bare flytte dem ind i ../mypaper
% hvis du føler for det
\newcommand\Cpp{C\raisebox{\height / 2}[\height][\depth]{\tiny ++}}
\usepackage{pdfpages} % Vi anvender \includepdf til at inkludere
                      % forsiden
\usepackage{array,booktabs}

\title{XY-plotter}
\author{Christian Klim Hansen \and Kristian Kjærgaard \and Nick Østergaard}
\date{\today}

\hypersetup{
  pdftitle={AceHigh, en (x,y)-plotter},
  pdfsubject={},
  pdfauthor={Christian Klim Hansen, Kristian Kjærgaard og Nick Østergaard},
  pdfcreator=LaTeX,
}

\pagestyle{Ruled}


\begin{document}

\includepdf{./img/forside}

\setcounter{page}{2}

\strut \vfill { \footnotesize
  \noindent 
  Copyright © 2009 Christian Klim Hansen, \\
  Kristian Kjærgaard og Nick Østergaard

  \medskip

  \noindent
  Layout af forfatterne med \LaTeX\\
  Illustrationer i Dia og Inkscape\\
  Billeder redigeret i GIMP\\
  Printdiagrammer og -udlæg i KiCad

  \medskip

  \noindent
  Hvor ikke andet angivet, tilhører\\
  ophavsretten forfatterne
}

\thispagestyle{empty}

\clearpage

\frontmatter

\tableofcontents

\include{./tex/forord}

\mainmatter

\include{./tex/indledning}
\include{./tex/brugervejledning}

\part{Design}
\label{prt:design}

\include{./tex/d-mekanik}
\include{./tex/d-hardware}
\include{./tex/d-software}

\part{Implementering}
\label{prt:implementering}

\include{./tex/i-mekanik}
\include{./tex/i-hardware}
\include{./tex/i-software}

% lortet (backmatter) virker ikke
%\backmatter

\include{./tex/afslutning}


\part{Bilag}
\appendix

\include{./tex/bilag-rettelser-software}
\include{./tex/bilag-kalibrering}
\include{./tex/bilag-sd-kort-adapter}
\include{./tex/bilag-l6208n-driver}
\include{./tex/bilag-hid}

\nocite{*}
\bibliographystyle{plain}
\bibliography{litteratur}
\label{ch:litteratur}

\listoffigures

\listoftables

\end{document}
